...
def is_categorical_data_uniform(data, name = None, alpha = .05):
    ...
    H0 = f"{modify_varname(name)}({name}) APPEARS to follow a balanced/uniform-like distribution (p: {p_value:.3f})."
    Ha = f"{modify_varname(name)}({name}) does NOT follow a balanced/uniform-like distribution (p: {p_value:.3f})."
...
def univariate_categorical_info(data):
  ...
  if n == 2:
    if majority_index in [1, 'Yes', 'yes', True]:
      return f'The majority are {modify_varname(data.name)}({data.name}) ({majority_proportion:.1f}%).'
    elif majority_index in [0, 'No', 'no', False]:
      return f'The majority are not {modify_varname(data.name)}({data.name}) ({majority_proportion:.1f}%).'
  if n > 3:
    return f'The most common value in {modify_varname(data.name)}({data.name}) is {majority_index} ({majority_proportion:.1f}%) followed by {proportions.index[1]} ({proportions.iloc[1]:.1f}%).'


  return f'The most common value in {modify_varname(data.name)}({data.name}) is {majority_index} ({majority_proportion:.1f}%).'
...
def is_numerical_data_normal(data):
  ...
  t = f'{modify_varname(name)}({name}) is {skew_description} (skewness: {data_skewness: .2f}) with average of {data.mean():.2f}.'
  print(t)

  if abs(data_skewness) < 0.5 and abs(data_kurtosis - 3) < 0.5:
    print(f"The data {modify_varname(name)}({name})  is LIKELY CLOSE to normal distribution.")
    return True
  else:
    print(f"The data {modify_varname(name)}({name})  may NOT be normally distributed.")
    return False
...
def get_outlier_percentage(data):
  ...
  print(f"Percentage of {modify_varname(name)}({name}) outliers using IQR: {percentage_outliers:.2f}%")
  ...
  print(f"Percentage of {modify_varname(name)}({name}) outliers using Z-score: {percentage_outliers_z:.2f}%")
...
def pairplot(data, vars = [], hue = None, palette = None,
  ...
  if hue:
    ...
    hue_name = camel_to_spaces(hue).replace(' ', '\n')
...
def pairplot_categorical(data, vars = [], hue = None, marker = 'o', alpha = 0.5,
  ...
  modify_name = lambda x: modify_varname(x).replace(' ', '\n')
  ...
  for i in range(n_cat):
    for j in range(n_cat):
      ...
      if j >= i:
        ...
        axs[j, i].set_xlabel(modify_name(x.name), rotation=rotation_x)
        axs[j, i].set_ylabel(modify_name(y.name), rotation=rotation_y)
...
def bivariate_categorical(df, categorical_cols, figsize=(4,4), color = 'skyblue',
  ...
  if len(categorical_cols) > 1:
    pairplot_categorical(df, rotation_x_tick=90, rotation_x=90, figsize=(13, 13))
...
def histplot_boxplot_hue(data, var,
  ...
  name = camel_to_spaces(var).replace(' ', '\n')
  ...
  title = f"{modify_varname(var)} & {modify_varname(hue)}" if hue else f"{modify_varname(var)}"
  ...
  axes[1].set_xlabel(camel_to_spaces(var).replace(' ', '\n'))
  ...
  if hue:
    axes[1].legend(frameon=False, loc='upper right', bbox_to_anchor=(1.5, 1),
                   fontsize = 12,
                   title=modify_varname(hue).replace(' ', '\n'))
